...
class AlexNetClassifier_Dynamic_TrainAll(nn.Module):
    def __init__(self, args, num_classes):
        ...
        self.classifier = nn.Sequential(
            nn.Linear(in_channels_at_depth * 3 * 3, 256),
            nn.ReLU(True),
            nn.Dropout(d),
            nn.Linear(256, num_classes)
        )
...
class MobileNetV2Classifier_Dynamic_Frozen(nn.Module):
    def __init__(self, args, num_classes=2):
        ...
        with torch.no_grad():
            dummy_input = torch.zeros(1, in_ch, 224, 224)
...
class ViTClassifier(nn.Module):
    def __init__(self, args, num_classes=1000):
        super().__init__()
        in_ch = args.sequence_length # 5
        
        config = ViTConfig(num_channels=in_ch, num_hidden_layers=3)        
...
class SwinClassifier(nn.Module):
    def __init__(self, args, num_classes=1000):
        super().__init__()
        in_ch = args.sequence_length # This is 5
...
class SwinClassifier_Dynamic_Frozen(nn.Module):
    def __init__(self, args, num_classes=2):
        super().__init__()
        in_ch = args.sequence_length # e.g., 5
...
class UNetPPClassifier(nn.Module):
    def __init__(self, args, num_classes=10):
        super().__init__()
        in_ch = args.sequence_length # This is 5
        self.unetpp = UnetPlusPlus(encoder_name='resnet34', encoder_weights=None, in_channels=in_ch, classes=1)
        self.fc = nn.Linear(512,num_classes)
    def forward(self,x):
        if len(x.shape) == 5:
            x = x.squeeze(1)  
...
class UNetResNetClassifier(nn.Module):
    def __init__(self, args, num_classes=10):
        ...
        self.unet = Unet(encoder_name='resnet34', encoder_weights=None, in_channels=in_ch, classes=1)
...
class AttentionUNetEfficientNet(nn.Module):
    def __init__(self, args, num_classes=10):
        ...
        # We set in_channels=in_ch (5) to match your input
